...
int main() {
    HeadingRecoveryConfig cfg; cfg.gain=1.0;
    ...
    std::mt19937 gen(93); std::uniform_real_distribution<double> d(-2,2);
    for(int i=0;i<10000;++i) {
        ...
        CHECK(std::abs((speed*speed+0.15*0.15)*result-speed*a-0.15*0.15*b)<1e-13);
        double angle=d(gen),c=std::cos(angle),s=std::sin(angle);
        auto rotated=headingRecoveryCentre(c*x-s*y,s*x+c*y,yaw+angle,0,0,0.15,cfg);
    ...
    bool threw=false; cfg.axis_bias=1;
    try { cfg.validate(); } catch(const std::invalid_argument&) { threw=true; }
    CHECK(threw);
    double lo,hi; cfg.axis_bias=0.5;
    CHECK(centredDampedYawRate(-1e-9,0.2,0.15,0.4,lo));
    CHECK(centredDampedYawRate( 1e-9,0.2,0.15,0.4,hi));
    CHECK(std::abs(hi-lo)<2e-8);
    std::cout<<"PASS: 10000 gradient/KKT/rotation cases, rest/lateral/reverse, damping, finite extremes, rejection, zero-speed continuity\n";
}
