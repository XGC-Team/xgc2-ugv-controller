...
struct HeadingRecoveryConfig {
    ...
    void validate() const {
        if (!std::isfinite(gain) || gain < 0.0 ||
            !std::isfinite(axis_bias) || axis_bias <= 0.0 || axis_bias >= 1.0 ||
            !std::isfinite(rate_damping) || rate_damping < 0.0) {
...
inline HeadingRecoveryResult headingRecoveryCentre(
    double hx, double hy, double yaw, double yaw_rate, double reference_rate,
    double eps, const HeadingRecoveryConfig& cfg) {
    cfg.validate();
    HeadingRecoveryResult out;
    if (!std::isfinite(hx) || !std::isfinite(hy) || !std::isfinite(yaw) ||
        !std::isfinite(yaw_rate) || !std::isfinite(reference_rate) ||
        !std::isfinite(eps) || eps <= 0.0) return out;
    if (cfg.gain == 0.0) { out.valid = true; return out; }
    ...
    out.centre = reference_rate + cfg.gain * out.descent
                 - cfg.rate_damping * (yaw_rate - reference_rate);
...
inline bool centredDampedYawRate(double speed, double a_perp, double eps,
                                 double centre, double& result) {
    result = 0.0;
    if (!std::isfinite(speed) || !std::isfinite(a_perp) ||
        !std::isfinite(eps) || eps <= 0.0 || !std::isfinite(centre)) return false;
...
inline bool regularizedReferenceRate(double vx, double vy, double ax, double ay,
                                     double eps, double& rate) {
    rate = 0.0;
    if (!std::isfinite(vx) || !std::isfinite(vy) || !std::isfinite(ax) ||
        !std::isfinite(ay) || !std::isfinite(eps) || eps <= 0.0) return false;
